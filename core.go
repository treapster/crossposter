...
<code>/add vk.com/group [id] [s]</code> - для приватных каналов без юзернейма, отправляй id канала. Ставь s в конце чтоб была ссылка на пост, id можно получить с помощью @my_id_bot.
